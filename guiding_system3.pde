...
void draw(){
  ...
  w = (int)relative_position.x - 200;
  q = (int)relative_position.y + 200;
  }
  
  
  
  if(keyPressed&&(key == CODED)&&(timer < 0)){    
    ...
    timer = 20;
    count_point = constrain(count_point, 0, 17);
  }
  String layer = "layer" + ":" + count_layer;
  String poi = "point" + ":" + count_point;
  text(layer, 100,50);
  text(poi, 200,50);
  ...
  for(int k = 0; k<count_layer+1; k++){
    ...
    if(k == count_layer){
      ...
      for(int l = 0; l<18; l++){
        ...
        float z1 = cordinate[k][l].z - 200;
        stroke(255,0,0);
        strokeWeight(2);
        float len = 1;
        line(w,q,200, w+len*x1, q,200);
        stroke(0,255,0);
        line(w,q,200, w, q+len*y1,200);
        stroke(0,0,255);
        line(w,q,200, w, q,200 + len*z1);        
        stroke(0,255,0);
        strokeWeight(9);
        }
        point(w,q,200);
